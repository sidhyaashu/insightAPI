...
def setup_production_logging(log_level: int = logging.INFO) -> None:
    ...
    Configures Rich colored console formatting and rotating file logging to logs/insightapi.log.
    """
    ...
    # 1. Rich Console Handler (for beautiful terminal UI logs)
    ...
    # 2. Rotating File Handler (written to logs/insightapi.log)
    ...
    for noisy_lib in ["urllib3", "asyncio", "playwright", "httpx", "httpcore"]:
        logging.getLogger(noisy_lib).setLevel(logging.WARNING)

    _logging_initialized = True
    logging.getLogger("insightapi.system").info(f"Production logging initialized. Log file: [bold cyan]{LOG_FILE_PATH}[/bold cyan]")
